...
The project owner explicitly instructed the agent to refine the opening summary protocol (`[Сильное решение]`, `[Нейтрально]`, `[Архитектурный риск]`). The owner indicated that forcing a rigid 3-bullet evaluation template on every single response turn is redundant and unhelpful when there are no major breakthroughs or genuine architectural risks.
...
Repeatedly outputting formal template blocks (`[Сильное решение]`, `[Нейтрально]`, `[Архитектурный риск]`) on routine, investigatory, or operational chat turns created unnecessary verbosity and mechanical routine. The owner requested an adaptive protocol:
1. Include `[Сильное решение]` only when the owner proposes an exceptionally strong/breakthrough architectural idea, explaining why so the owner can build upon it.
2. Include `[Архитектурный риск / Пушбэк]` whenever architectural, consistency, performance, or technical risks exist.
...
Accepted by the owner on 2026-08-17. Implemented in `guide-architecture-design/references/decision-capture-and-sync.md` under `Adapt opening evaluation summary`, omitting rigid 3-bullet evaluation blocks on routine turns while retaining `[Сильное решение]` for breakthrough proposals and `[Архитектурный риск / Пушбэк]` for identified risks.
